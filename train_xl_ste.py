...
def patch_linear_module_for_ste(module, n_bits=4, step=0.1/7):
    """
...
def apply_ste_to_target_layers(model, n_bits=4, step=0.1/7):
    patched = []
    for name, module in model.named_modules():
        if is_target_linear_name(name):
...
apply_ste_to_target_layers(model, n_bits=4, step=0.1/7)
