...
from qat_prox2.quant_ops import hard_quantize_model_inplace, restore_model_from_backup, selective_hard_quantize_model_inplace
from qat_prox2.quant_stats import compute_quantization_rate_fast
from qat_prox2.utils4 import prepare_theory_matched_quant_update, apply_prepared_quant_update
...
def estimate_loss():
    ...
    backup_hard = hard_quantize_model_inplace(
        model, qat.n_bits_w, qat.step_w,
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            losses[k] = loss.item()
        out[f'{split}_hard'] = losses.mean()
    restore_model_from_backup(model, backup_hard)


    backup_sel = selective_hard_quantize_model_inplace(
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            with ctx:
                logits, loss = model(X, Y)
            losses[k] = loss.item()
        out[f'{split}_sel'] = losses.mean()

    restore_model_from_backup(model, backup_sel)
    model.train()
    return out
...
# Configuration parameters: 4-bit, range [-0.05, 0.05]
# L = 2**(4-1) - 1 = 7. step = 0.05 / 7
qat_step_size = 0.05 / 7.0
qat = QATConfig()
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        print(f"  --> [地狱/Hard] train loss {losses['train_hard']:.4f}, val loss {losses['val_hard']:.4f}")
        print(f"  --> [温柔/Semi] train loss {losses['train_sel']:.4f}, val loss {losses['val_sel']:.4f}")
        
        track_val_loss = losses['val_hard']
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        # 1. Backup true weights, convert to quantized weights (clamped between -0.05 and 0.05)
        #backup_q = hard_quantize_model_inplace(model, qat.n_bits_w, qat.step_w, selector=q_selector, exclude_substrings=qat.exclude_substrings)
        backup_q = selective_hard_quantize_model_inplace(
            model, 
            qat.n_bits_w, 
            qat.step_w, 
            atol=qat.atol, 
        ...
        # 2. Run forward pass with quantized weights
        with ctx:
            logits, loss = model(X, Y)
        ...
        # 4. Restore weights back to high precision FP32/BF16 for optimizer updates
        restore_model_from_backup(model, backup_q)
        
        # 5. Calculate unquantized loss at the last micro-step for the dual controller to work
    ...
    quant_step_pkg = prepare_theory_matched_quant_update(
        model, optimizer, qat=qat, selector=q_selector, dual_lambda=dual_lambda
    )
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        print(f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}% | lambda: {dual_lambda:.4f}, qrate: {current_qrate*100:.2f}%")
