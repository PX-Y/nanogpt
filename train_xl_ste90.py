...
from ste_quant2 import ste_quantize_weight_ratio, quantize_to_fixed_grid
...
def patch_linear_module_for_ste(module, n_bits=4, step=0.1/7, ratio=0.9):
    if hasattr(module, "_ste_patched") and module._ste_patched:
        return

    def ste_forward(self, x):
        w_ste = ste_quantize_weight_ratio(self.weight, n_bits=n_bits, step=step, ratio=ratio)
...
def apply_ste_to_target_layers(model, n_bits=4, step=0.1/7, ratio=0.9):
    patched = []
    for name, module in model.named_modules():
        if is_target_linear_name(name):
            if not hasattr(module, "weight"):
                continue
            patch_linear_module_for_ste(module, n_bits=n_bits, step=step, ratio=ratio)
            patched.append(name)

    print(f"[STE] patched {len(patched)} modules (ratio={ratio})")
    for n in patched:
        print("  ", n)
...
apply_ste_to_target_layers(model, n_bits=4, step=0.1/7, ratio=0.9)
