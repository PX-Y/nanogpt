...
from ste_quant import ste_quantize_weight, quantize_to_fixed_grid
...
def is_target_linear_name(name: str) -> bool:
    return (
        name.endswith("attn.c_attn")
        or name.endswith("attn.c_proj")
        or name.endswith("mlp.c_fc")
        or name.endswith("mlp.c_proj")
    )

def print_target_layers(model):
    print("==== target quant layers ====")
    for name, module in model.named_modules():
        if is_target_linear_name(name):
            print(name, type(module))
    print("=============================")


def patch_linear_module_for_ste(module, n_bits=4, step=0.05/7):
    """
    Monkey-patch a linear-like module so that forward uses STE-quantized weight.
    Assumes module has .weight and optional .bias and behaves like F.linear.
    """
    if hasattr(module, "_ste_patched") and module._ste_patched:
        return

    def ste_forward(self, x):
        w_ste = ste_quantize_weight(self.weight, n_bits=n_bits, step=step)
        return F.linear(x, w_ste, self.bias)

    module.forward = types.MethodType(ste_forward, module)
    module._ste_patched = True

def apply_ste_to_target_layers(model, n_bits=4, step=0.05/7):
    patched = []
    for name, module in model.named_modules():
        if is_target_linear_name(name):
            if not hasattr(module, "weight"):
                print(f"[skip] {name} has no weight")
                continue
            patch_linear_module_for_ste(module, n_bits=n_bits, step=step)
            patched.append(name)

    print(f"[STE] patched {len(patched)} modules")
    for n in patched:
        print("  ", n)
...
print_target_layers(model)
apply_ste_to_target_layers(model, n_bits=4, step=0.05/7)
